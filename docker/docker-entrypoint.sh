...
if [ "$1" = 'rollback' ]; then
  ...
  checkConnection
  # Configure server
  su digithinkit -c "bash -c configureServer"
  # Rollback Migrations
  echo "Rollback migrations"
  # su digithinkit -c "./node_modules/.bin/migrate down updateRoleScopeUuid -d mongodb://$DB_HOST:27017/$DB_NAME"
fi
...
if [ "$1" = 'start' ]; then
  ...
  # Run Migrations
  echo "Run migrations"
  # su digithinkit -c "./node_modules/.bin/migrate up -d mongodb://$DB_HOST:27017/$DB_NAME"
